...
def create_app():
    ...
    if app.config.get("DB_PASSWORD"):
        app.config["SQLALCHEMY_DATABASE_URI"] = app.config.get("SQLALCHEMY_DATABASE_URI", "").replace(
    ...
    app.config["FRONTEND_URL"] = os.environ.get(
        "HINKSKALLE_FRONTEND_URL", app.config.get("FRONTEND_URL", app.config["BACKEND_URL"])
    )
    ...
    app.config["ENABLE_REGISTER"] = os.environ.get(
        "HINKSKALLE_ENABLE_REGISTER", app.config.get("ENABLE_REGISTER", False)
    )
    app.config["SINGULARITY_FLAVOR"] = os.environ.get(
        "HINKSKALLE_SINGULARITY_COMMAND", app.config.get("SINGULARITY_FLAVOR", "singularity")
    )
    app.config["DEFAULT_USER_QUOTA"] = os.environ.get(
        "HINKSKALLE_DEFAULT_USER_QUOTA", app.config.get("DEFAULT_USER_QUOTA", 0)
    )
    app.config["DEFAULT_GROUP_QUOTA"] = os.environ.get(
        "HINKSKALLE_DEFAULT_GROUP_QUOTA", app.config.get("DEFAULT_GROUP_QUOTA", 0)
    )
