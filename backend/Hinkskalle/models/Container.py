...
class Container(db.Model):  # type: ignore
    ...
    images_ref = db.relationship(
        "Image", back_populates="container_ref", lazy="dynamic", cascade="all, delete-orphan"
    )
    ...
    def type(self) -> str:
        media_types = (
            ...
            .filter(Image.container_id == self.id, Image.hide == False)
    ...
    def imageTags(self) -> dict:
        ...
        for tag in Tag.query.filter(
            Tag.container_ref == self, or_(Tag.arch == None, Tag.arch == current_app.config["DEFAULT_ARCH"])
        ).all():
