...
class ImageUploadUrl(db.Model):  # type: ignore
    ...
    parts_ref = db.relationship(
        "ImageUploadUrl", back_populates="parent_ref", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class Image(db.Model):  # type: ignore
    ...
    uploads_ref = db.relationship(
        "ImageUploadUrl", back_populates="image_ref", lazy="dynamic", cascade="all, delete", passive_deletes=True
    )
    ...
    def type(self) -> str:
        if self.media_type == "application/vnd.docker.image.rootfs.diff.tar.gzip":
            ...
            self.media_type == "application/vnd.oci.image.layer.v1.tar+gzip"
            or self.media_type == "application/vnd.oci.image.layer.v1.tar"
        ):
    ...
    def _get_signature(self) -> dict:
        ...
        proc = subprocess.run(
            ["singularity", "verify", "--url", current_app.config["KEYSERVER_URL"], "--json", self.location],
