...
class UserGroup(db.Model):  # type: ignore
    ...
    role = db.Column(
        "role", db.Enum(GroupRoles, name="group_roles"), default=GroupRoles.readonly.value, nullable=False
    )
...
class User(db.Model):  # type: ignore
    ...
    manual_tokens = db.relationship(
        "Token", viewonly=True, primaryjoin="and_(User.id==Token.user_id, Token.source=='manual')"
    )
...
class Group(db.Model):  # type: ignore
    ...
    def _valid_images(self):
        ...
        return Image.query.join(Container, Collection, Entity).filter(
            Entity.group == self, Image.uploadState == UploadStates.completed
        )
