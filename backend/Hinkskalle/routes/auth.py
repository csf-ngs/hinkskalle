...
def token_status():
    ...
    return {"status": "welcome", "data": g.authenticated_user}
        ...
        encoded_jwt = jwt.encode(
            {
                ...
                "exp": data.get(
                    "exp", timegm(datetime.utcnow().utctimetuple()) + current_app.config["DOWNLOAD_TOKEN_EXPIRATION"]
                ),
            },
...
def get_authn_create_options():
    ...
    opts = generate_registration_options(
        ...
        exclude_credentials=[PublicKeyCredentialDescriptor(id=key.id) for key in g.authenticated_user.passkeys],
        timeout=180000,
    )
...
@registry.handles(
    rule="/v1/webauthn/signin", method="POST", response_body_schema=GetTokenResponseSchema(), tags=["hinkskalle-ext"]
)
def authn_signin():
