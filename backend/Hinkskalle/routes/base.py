...
@registry.handles(rule="/version", method="GET", response_body_schema=VersionResponseSchema(), tags=["singularity"])
def version():
...
def config():
    ...
    return {
        "libraryAPI": {"uri": service_url},
        "keystoreAPI": {
            "uri": current_app.config.get("KEYSERVER_URL"),
        },
        "tokenAPI": {"uri": service_url},
        "params": {
...
def create_error_object(code, msg):
    return [{"title": "Fail!", "detail": msg, "code": code}]
