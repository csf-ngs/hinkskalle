...
def push_image_v2_init(image_id):
    ...
    return {"data": {"uploadURL": upload_url}}
...
def push_image_v2_multi_part(image_id):
    ...
    part = ImageUploadUrl.query.filter(
        ImageUploadUrl.partNumber == body.get("partNumber"), ImageUploadUrl.parent_ref == upload
    ).first()
...
@registry.handles(rule="/v2/imagefile/_upload/<string:upload_id>", method="PUT", tags=["hinkskalle-ext"])
...
def push_image_v2_complete(image_id):
    ...
    image = _get_image_id(image_id)
    upload = (
        image.uploads_ref.filter(
            ImageUploadUrl.state == UploadStates.uploaded, ImageUploadUrl.type == UploadTypes.single
        )
        .order_by(ImageUploadUrl.createdAt.desc())
        .first()
    )
    if not upload:
...
def _check_quota(image: Image, add_size=None) -> bool:
    ...
    if image.owner and image.owner.quota > 0 and image.owner.used_quota + check_size >= image.owner.quota:
        return False
    elif entity.group and entity.group.quota > 0 and entity.used_quota + check_size >= entity.group.quota:
        return False
    else:
        return True
