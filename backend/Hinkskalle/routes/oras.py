...
def handle_oras_error(error: OrasError):
    body = {"errors": [{"code": error.code, "message": error.message, "detail": error.detail}]}
    resp = jsonify(body)
...
@registry.handles(
    rule="/v2/", method="GET", authenticators=authenticator.with_scope(Scopes.optional), tags=["oci"]  # type: ignore
)
...
@registry.handles(rule="/v2/", method="POST", tags=["oci"])
def authenticate():
...
def oras_list_tags(name: str):
    ...
    if args.get("n") is None or args.get("n") > 0:
        cur_tags: List[str] = [
            t.name
            for t in Tag.query.filter(Tag.image_id.in_([i.id for i in container.images_ref])).order_by(Tag.name)
        ]
    ...
    return {"name": f"{container.entityName}/{container.collectionName}/{container.name}", "tags": cur_tags}
...
def oras_manifest(name: str, reference: str):
    ...
    if reference.startswith("sha256:"):
        try:
            manifest = Manifest.query.filter(
                Manifest.hash == reference.replace("sha256:", ""), Manifest.container_ref == container
            ).one()
...
def oras_push_manifest(name, reference):
        ...
        for layer in manifest_data.get("layers", []) + [manifest_data.get("config", {})]:
            ...
            try:
                image = Image.query.filter(
                    Image.hash == layer.get("digest").replace("sha256:", "sha256."), Image.container_ref == container
                ).one()
    ...
    manifest_url = (
        _get_service_url()
        + f"/v2/{container.entityName}/{container.collectionName}/{container.name}/manifests/sha256:{manifest.hash}"
    )
...
def oras_start_upload_session(name):
        ...
        try:
            ...
            try:
                ...
                if not entity.check_update_access(g.authenticated_user):
                    current_app.logger.debug(
                        f"User {g.authenticated_user.username} tried to create {entity_id}/{collection_id}"
                    )
            ...
            try:
                ...
                if not collection.check_update_access(g.authenticated_user):
                    current_app.logger.debug(
                        f"User {g.authenticated_user.username} tried to create {entity_id}/{collection_id}/{container_id}"
                    )
    ...
    if args.get("staged", False) or (
        headers.get("content_type") == "application/octet-stream" and int(headers.get("content_length", "0")) > 0
    ):
...
def _do_single_post_upload(upload: ImageUploadUrl, digest: str, staged: bool = False):
    ...
    blob_url = (
        _get_service_url()
        + f"/v2/{image.container_ref.entityName}/{image.container_ref.collectionName}/{image.container_ref.name}/blobs/{image.hash.replace('sha256.', 'sha256:')}"
    )
    response = make_response("", 201)
...
def _do_mount(container: Container, _from: str, mount: str):
    ...
    try:
        from_image = Image.query.filter(
            Image.container_id == from_container.id, Image.hash == mount.replace("sha256:", "sha256.")
        ).one()
    except NoResultFound:
...
@registry.handles(rule="/v2/__uploads/<string:upload_id>", method="PATCH", tags=["hinkskalle-ext"])
...
@registry.handles(rule="/v2/__uploads/<string:upload_id>/<string:chunk_id>", method="PATCH", tags=["hinkskalle-ext"])
...
def oras_push_chunk_finish(upload_id, chunk_id):
    ...
    blob_url = (
        _get_service_url()
        + f"/v2/{upload.image_ref.entityName}/{upload.image_ref.collectionName}/{upload.image_ref.containerName}/blobs/{upload.image_ref.hash.replace('sha256.', 'sha256:')}"
    )
    response = make_response("", 201)
    response.headers["Location"] = blob_url
...
def _get_chunk(upload_id: str, chunk_id: str) -> Tuple[ImageUploadUrl, ImageUploadUrl]:
    ...
    try:
        chunk = ImageUploadUrl.query.filter(
            ImageUploadUrl.id == chunk_id, ImageUploadUrl.parent_id == upload.id
        ).one()
    except NoResultFound:
        raise OrasBlobUploadUnknown()
...
def _receive_upload(upload: ImageUploadUrl, digest: str, staged: bool = False) -> Image:
    ...
    if (
        not staged
        and request.headers.get("content_length") is not None
        and read != int(request.headers["content_length"])
    ):
