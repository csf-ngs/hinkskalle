...
def _create_user(body, password=None):
    ...
    try:
        ...
        msg = (
            f"User {new_user.username} already exists (email and/or username already taken"
            if err.statement.find("entity") == -1
            else f"entity {new_user.username} already exists"
        )
...
def update_user(username):
    ...
    if user.source != "local":
        body = {
            k: v for k, v in body.items() if k in ["is_admin", "is_active", "source", "quota", "password_disabled"]
        }
