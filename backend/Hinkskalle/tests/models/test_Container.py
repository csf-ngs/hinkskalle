...
class TestContainer(ModelBase):
    ...
    def test_tag_image(self):
        ...
        self.assertListEqual([f"{tag.name}:{tag.image_ref.id}" for tag in tags], [f"v1:{image1.id}"])
        ...
        self.assertListEqual(
            [f"{tag.name}:{tag.image_ref.id}" for tag in tags], [f"v1:{image1.id}", f"v1.1:{image1.id}"]
        )
    ...
    def test_tag_image_arch(self):
        ...
        self.assertDictEqual(container.archImageTags, {"c64": {"v1": str(image1.id)}})
    ...
    def test_get_arch_tags(self):
        ...
        self.assertDictEqual(container.archImageTags, {"powerpc": {"v1": str(image1.id)}})
