...
class TestOrasPush(RouteBase):
    ...
    def test_push_monolith_get_session(self):
        ...
        self.assertTrue(
            abs(db_upload.expiresAt - (datetime.datetime.now() + datetime.timedelta(minutes=5)))
    ...
    def test_push_monolith_create_user(self):
        ...
        self.assertEqual(ret.status_code, 403)
        self.assertIsNone(
            Collection.query.filter(Collection.name == "hase", Collection.entity_id == entity.id).first()
        )
        ...
        with self.fake_auth():
            ret = self.client.post(
                f"/v2/{entity.name}/{collection.name}/stall/blobs/uploads/", headers={"Content-Length": 0}
            )
        self.assertEqual(ret.status_code, 403)
        self.assertIsNone(
            Container.query.filter(Container.name == "stall", Container.collection_id == collection.id).first()
        )
    ...
    def test_push_monolith_create_set_expiration(self):
        ...
        with self.fake_auth():
            ret = self.client.post(
                f"/v2/{self.username}/hase/stall/blobs/uploads/?expiresAt={exp}", headers={"Content-Length": 0}
            )
        print(ret.get_json())
        self.assertEqual(ret.status_code, 202)
    ...
    def test_push_monolith_create_set_private(self):
        with self.fake_auth():
            ret = self.client.post(
                f"/v2/{self.username}/hase/stall/blobs/uploads/?private=true", headers={"Content-Length": 0}
            )
        self.assertEqual(ret.status_code, 202)
    ...
    def test_push_monolith_create_set_private_existing(self):
        ...
        with self.fake_auth():
            ret = self.client.post(
                f"/v2/{self.username}/{collection.name}/{container.name}/blobs/uploads/?private=true",
    ...
    def test_push_monolith_mount(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.post(
                f"/v2/{image1.entityName}/{image1.collectionName}/{image1.containerName}/blobs/uploads/?mount={image2.hash.replace('sha256.', 'sha256:')}&from={image2.entityName}/{image2.collectionName}/{image2.containerName}"
            )
        ...
        self.assertRegexpMatches(
            ...
            rf"/{image1.entityName}/{image1.collectionName}/{image1.containerName}/blobs/{digest}",
        )
    ...
    def test_push_monolith_mount_not_found(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.post(
                f"/v2/{image1.entityName}/{image1.collectionName}/{image1.containerName}/blobs/uploads/?mount={image2.hash.replace('sha256.', 'sha256:')}&from={image2.entityName}/{image2.collectionName}oink/{image2.containerName}"
            )
        self.assertEqual(ret.status_code, 404)

        with self.fake_admin_auth():
            ret = self.client.post(
                f"/v2/{image1.entityName}/{image1.collectionName}/{image1.containerName}/blobs/uploads/?mount={image2.hash.replace('sha256.', 'sha256:')}oink&from={image2.entityName}/{image2.collectionName}/{image2.containerName}"
            )
        self.assertEqual(ret.status_code, 404)
    ...
    def test_push_monolith_get_session_existing(self):
        ...
        self.assertEqual(ret.status_code, 202)
        db_coll = Collection.query.filter(
            Collection.name == f"{coll_name}oink", Collection.entity_id == entity_id
        ).first()
        ...
        self.assertEqual(ret.status_code, 202)
        db_container = Container.query.filter(
            Container.name == f"{container_name}oink", Container.collection_id == coll_id
        ).first()
    ...
    def test_push_monolith_get_session_readonly(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.post(
                f"/v2/{image.entityName}/{image.collectionName}/{image.containerName}/blobs/uploads/"
            )
        self.assertEqual(ret.status_code, 403)
    ...
    def test_push_monolith_do(self):
        ...
        with mock.patch("Hinkskalle.routes.imagefiles._make_filename") as mock_make_fn:
            ...
            ret = self.client.put(
                f"/v2/__uploads/{upload.id}?digest={digest}", data=img_data, content_type="application/octet-stream"
            )
        self.assertEqual(ret.status_code, 201)
        self.assertRegexpMatches(
            ret.headers.get("location", ""), rf"/{entity.name}/{collection.name}/{container.name}/blobs/{digest}"
        )
        self.assertIsNotNone(ret.headers.get("location"))
    ...
    def test_push_monolith_checksum_mismatch(self):
        ...
        digest = digest.replace("sha256.", "sha256:")
        ret = self.client.put(
            f"/v2/__uploads/{upload.id}?digest={digest}oink", data=img_data, content_type="application/octet-stream"
        )
    ...
    def test_push_single_post(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.post(
                f"/v2/{image.entityName}/{image.collectionName}/{image.containerName}/blobs/uploads/?digest={digest}",
                data=img_data,
    ...
    def test_push_single_post_stage(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.post(
                f"/v2/{image.entityName}/{image.collectionName}/{image.containerName}/blobs/uploads/?digest={digest}&staged=1",
                content_type="application/octet-stream",
            )
        print(ret.get_json())
        self.assertEqual(ret.status_code, 201)
    ...
    def test_push_manifest_noauth(self):
        ...
        ret = self.client.put(
            f"/v2/{entity.name}/{collection.name}/{container.name}/manifests/v2", json=test_manifest
        )
        self.assertEqual(ret.status_code, 401)
    ...
    def test_push_manifest_readonly(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.put(
                f"/v2/{image.entityName}/{image.collectionName}/{image.containerName}/manifests/v2",
                json=test_manifest,
            )
        self.assertEqual(ret.status_code, 403)
    ...
    def test_push_manifest_existing_other_tag(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.put(
                f"/v2/{entity.name}/{collection.name}/{container.name}/manifests/other", data=test_manifest_text
            )
        self.assertEqual(ret.status_code, 201)

        digest: str = ret.headers.get("Docker-Content-Digest", "")
        db_manifest = Manifest.query.filter(Manifest.hash == digest.replace("sha256:", "")).one()
        self.assertCountEqual([t.name for t in db_manifest.tags], ["v2", "other"])
    ...
    def test_push_manifest_not_found(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.put(
                f"/v2/{entity.name}/{collection.name}/{container.name}oink/manifests/v2", json=test_manifest
            )
        self.assertEqual(ret.status_code, 404)
