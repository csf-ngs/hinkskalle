...
class TestOrasPushChunked(RouteBase):
    def test_push_chunks_get_session(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.post(
                f"/v2/{container.entityName}/{container.collectionName}/{container.name}/blobs/uploads/",
    ...
    def test_push_chunks_init(self):
        ...
        upload = ImageUploadUrl(
            image_ref=image, state=UploadStates.initialized, type=UploadTypes.undetermined, path="/test/hase"
        )
        ...
        with self.fake_admin_auth():
            ret = self.client.patch(
                f"/v2/__uploads/{upload.id}", data="oink", headers={"Content-Type": "application/octet-stream"}
            )
        self.assertEqual(ret.status_code, 202)
    ...
    def test_push_chunks_init_content_range(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.patch(
                f"/v2/__uploads/{upload.id}",
                data="oink",
                headers={"Content-Type": "application/octet-stream", "Content-Range": "0-3"},
            )
        self.assertEqual(ret.status_code, 202)
    ...
    def test_push_chunks_init_content_range_invalid_begin(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.patch(
                f"/v2/__uploads/{upload.id}",
                data="oink",
                headers={"Content-Type": "application/octet-stream", "Content-Range": "3-99"},
            )
    ...
    def test_push_chunks_init_content_range_invalid_length(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.patch(
                f"/v2/__uploads/{upload.id}",
                data="oink",
                headers={"Content-Type": "application/octet-stream", "Content-Range": "0-5"},
            )
    ...
    def test_push_chunks_continue(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.patch(
                f"/v2/__uploads/{upload.id}/{chunk.id}",
                data="oink",
    ...
    def test_push_chunk_finish(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.put(
                f"/v2/__uploads/{upload.id}/{last_chunk.id}?digest={complete_digest.replace('sha256.', 'sha256:')}"
            )
    ...
    def test_push_chunk_finish_checksum(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.put(
                f"/v2/__uploads/{upload.id}/{last_chunk.id}?digest={complete_digest.replace('sha256.', 'sha256:')}oink"
            )
        self.assertEqual(ret.status_code, 400)
