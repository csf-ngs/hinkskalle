...
class TestCollections(RouteBase):
    ...
    def test_create_user_default_entity_reserved(self):
        ...
        db.session.commit()
        with self.fake_auth():
            ret = self.client.post("/v1/collections", json={"name": "", "entity": str(entity.id)})
        self.assertEqual(ret.status_code, 403, "empty name")

        with self.fake_auth():
            ret = self.client.post("/v1/collections", json={"name": "default", "entity": str(entity.id)})
        self.assertEqual(ret.status_code, 403, "default")

        with self.fake_auth():
            ret = self.client.post("/v1/collections", json={"name": "pipeline", "entity": str(entity.id)})
    ...
    def test_update(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.put(
                f"/v1/collections/{entity.name}/{coll.name}",
                json={
    ...
    def test_update_case(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.put(
                "/v1/collections/Test-Hase/TestHase",
                json={
                    "description": "Mei Huat",
                },
            )
    ...
    def test_update_owner(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.put(
                f"/v1/collections/{entity.name}/{coll.name}", json={"createdBy": self.other_username}
            )
    ...
    def test_update_user_owner(self):
        ...
        with self.fake_auth():
            ret = self.client.put(f"/v1/collections/{entity.name}/{coll.name}", json={"createdBy": self.username})
        self.assertEqual(ret.status_code, 200)
