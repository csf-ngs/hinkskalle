...
class TestContainers(RouteBase):
    ...
    def test_update(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.put(
                f"/v1/containers/{entity.name}/{coll.name}/{container.name}",
                json={
    ...
    def test_update_case(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.put(
                f"/v1/containers/Test-Hase/test-Collection-Container/Test-Container",
                json={
                    "description": "Mei Huat",
                },
            )
    ...
    def test_update_owner(self):
        ...
        with self.fake_admin_auth():
            ret = self.client.put(
                f"/v1/containers/{entity.name}/{coll.name}/{container.name}", json={"createdBy": self.other_username}
            )
    ...
    def test_update_user_owner(self):
        ...
        with self.fake_auth():
            ret = self.client.put(
                f"/v1/containers/{entity.name}/{coll.name}/{container.name}", json={"createdBy": self.username}
            )
