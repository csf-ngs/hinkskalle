...
from .._util import _create_group, _set_member, default_entity_name, _get_json_data
...
class TestEntities(RouteBase):
    ...
    def test_get_case(self):
        ...
        self.assertEqual(ret.status_code, 200)
        self.assertEqual(_get_json_data(ret)["id"], str(entity.id))
    ...
    def test_create_behalf(self):
        with self.fake_admin_auth():
            ret = self.client.post("/v1/entities", json={"name": self.username})
        self.assertEqual(ret.status_code, 200)
    ...
    def test_create_user_name_check(self):
        with self.fake_auth():
            ret = self.client.post("/v1/entities", json={"name": "grunzuser"})
        self.assertEqual(ret.status_code, 403)

        with self.fake_auth():
            ret = self.client.post("/v1/entities", json={"name": "default"})
        self.assertEqual(ret.status_code, 403)

        with self.fake_auth():
            ret = self.client.post("/v1/entities", json={"name": ""})
        self.assertEqual(ret.status_code, 403)
    ...
    def test_update_case(self):
        ...
        db.session.commit()
        with self.fake_admin_auth():
            ret = self.client.put("/v1/entities/TestHase", json={"description": "Oink oink"})
        self.assertEqual(ret.status_code, 200)
