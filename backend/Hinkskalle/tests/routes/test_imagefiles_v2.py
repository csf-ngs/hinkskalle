...
from .._util import _create_image, _create_user, _get_json_data
...
class TestImagefilesV2(RouteBase):
    def test_push_v2_single(self):
        ...
        img_data = {"filesize": 1, "sha256sum": "something", "md5sum": "also something"}
    ...
    def test_push_v2_single_quota_check(self):
        ...
        img_data = {"filesize": 2048, "sha256sum": "something", "md5sum": "also something"}
    ...
    def test_push_v2_multi_init_user(self):
        ...
        self.assertEqual(ret.status_code, 200)
        db_upload = ImageUploadUrl.query.filter(ImageUploadUrl.id == _get_json_data(ret).get("uploadID")).first()
    ...
    def test_push_v2_multi_part_twice(self):
        ...
        self.assertEqual(ret.status_code, 200)
        first_url = _get_json_data(ret).get("presignedURL")

        with self.fake_admin_auth():
            ret = self.client.put(f"/v2/imagefile/{image_id}/_multipart", json=part_data)

        self.assertEqual(ret.status_code, 200)
        self.assertEqual(_get_json_data(ret).get("presignedURL"), first_url)
    ...
    def test_push_v2_multi_complete(self):
        ...
        complete_json = {
            "uploadID": upload.id,
            "completedParts": [{"partNumber": p.partNumber, "token": p.sha256sum} for p in parts],
        }
    ...
    def test_push_v2_multi_complete_quota(self):
        ...
        data = _get_json_data(ret)
        self.assertDictEqual(data.get("quota", {}), {"quotaTotal": 0, "quotaUsage": len(complete_data)})
    ...
    def test_push_v2_multi_complete_quota_signal(self):
        ...
        data = _get_json_data(ret)
        self.assertDictEqual(
            data.get("quota", {}), {"quotaTotal": len(complete_data) * 2, "quotaUsage": len(complete_data)}
        )
    ...
    def test_push_v2_multi_complete_file_missing(self):
        ...
        db_parts = ImageUploadUrl.query.filter(ImageUploadUrl.parent_id == upload_id).order_by(
            ImageUploadUrl.partNumber.asc()
        )
