...
class TestConfig(unittest.TestCase):
  ...
  def test_db_config(self):
    ...
    test_app = create_app()
    self.assertEqual(test_app.config.get('SQLALCHEMY_DATABASE_URI'), 'postgresql+psycopg2://knihskalle:supersecret@hinkdb/hinkskalle')

    os.environ['DB_PASSWORD'] = 'anothersecret'
    test_app = create_app()
    self.assertEqual(test_app.config.get('SQLALCHEMY_DATABASE_URI'), 'postgresql+psycopg2://knihskalle:anothersecret@hinkdb/hinkskalle')

    os.environ['SQLALCHEMY_DATABASE_URI'] = 'postgresql+psycopg2://oink:%PASSWORD%@oink/oink'
    test_app = create_app()
    self.assertEqual(test_app.config.get('SQLALCHEMY_DATABASE_URI'), 'postgresql+psycopg2://oink:anothersecret@oink/oink')
