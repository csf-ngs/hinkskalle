...
class MockLDAP:
    ...
    def __init__(self, filter=LDAPUsers.default_filter, all_users_filter=LDAPUsers.default_all_users_filter):
        ...
        self.svc.connection.strategy.add_entry(
            self.dummy_root_cn, {"cn": "Root Hase", "uid": self.dummy_root, "userPassword": self.dummy_password}
        )
...
class TestLdap(ModelBase):
    ...
    def test_config_additional(self):
        dummy_cfg = {
            "ENABLED": True,
            "FILTERS": {"user": "(hase={})", "all_users": "(objectClass=ziege)"},
            "ATTRIBUTES": {"username": "oink", "email": "muh", "firstname": "bär", "lastname": "hase"},
        }
        saved = self.app.config.get("AUTH", {})
        self.app.config["AUTH"] = {"LDAP": dummy_cfg}
        auth = LDAPUsers()
    ...
    def test_sync_existing(self):
        ...
        db_user = auth.sync_user(
            {
                "attributes": {
                    "cn": f"{user.firstname} f{user.lastname}",
                    "uid": user.username,
                    "mail": user.email,
        ...
        db_user = auth.sync_user(
            {
                "attributes": {
                    ...
                    "uid": user.username,
                    "mail": user.email,
                    "givenName": user.firstname + "oink",
                    "sn": user.lastname + "oink",
                }
            }
        )
        self.assertEqual(db_user.id, user.id)
    ...
    def test_db_sync(self):
        ...
        self.assertDictContainsSubset({"job": job.id, "synced": ["test.hase"], "conflict": [], "failed": []}, key.val)
