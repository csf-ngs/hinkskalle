...
from flask_rebar.swagger_generation.authenticator_to_swagger import AuthenticatorConverterRegistry, AuthenticatorConverter
...
class TokenAuthConverter(AuthenticatorConverter):
    ...
    def get_security_schemes(self, obj, context=None):
        return {
            obj.name: {sw.type_: sw.api_key, sw.in_: TokenAuthenticator.header, sw.name: TokenAuthenticator.type}
        }
