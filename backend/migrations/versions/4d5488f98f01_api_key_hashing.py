...
def upgrade():
    ...
    with op.batch_alter_table("token") as batch_op:
        batch_op.add_column(sa.Column("key_uid", sa.String(), nullable=True))
        batch_op.drop_constraint("uq_token_token", type_="unique")
        batch_op.create_unique_constraint(op.f("uq_token_key_uid"), ["key_uid"])
    conn = op.get_bind()
    ...
    for row in res.fetchall():
        ...
        upd = conn.execute(
            text("UPDATE token SET key_uid=:key_uid, token=:token WHERE id=:id"),
            id=row[0],
            key_uid=key_uid,
            token=token,
        )
    ...
    with op.batch_alter_table("token") as batch_op:
        batch_op.alter_column("key_uid", nullable=False)
