...
def upgrade():
    ...
    with op.batch_alter_table("tag") as batch_op:
        ...
        batch_op.add_column(sa.Column("container_id", sa.Integer()))
        batch_op.drop_constraint("name_image_id_idx", type_="unique")
        batch_op.create_unique_constraint("tag_name_container_arch_idx", ["name", "container_id", "arch"])
        batch_op.create_foreign_key(op.f("fk_tag_container_id_container"), "container", ["container_id"], ["id"])
    conn.execute(
        sa.text(
            "UPDATE tag SET container_id=(SELECT container_id FROM image WHERE image.id=tag.image_id), arch=(SELECT arch FROM image WHERE image.id=tag.container_id)"
        )
    )
    with op.batch_alter_table("tag") as batch_op:
    ...
    with op.batch_alter_table("manifest") as batch_op:
        batch_op.add_column(sa.Column("container_id", sa.Integer()))
        batch_op.drop_constraint("uq_manifest_hash", type_="unique")
        batch_op.create_unique_constraint("manifest_hash_container_idx", ["hash", "container_id"])
        batch_op.create_foreign_key(op.f("fk_manifest_container_id_container"), "container", ["container_id"], ["id"])
    conn.execute(
        sa.text(
            "UPDATE manifest SET container_id=(SELECT min(container_id) FROM tag WHERE tag.manifest_id=manifest.id)"
        )
    )
