...
def upgrade():
    ...
    users_table = sa.sql.table(
        "user", sa.sql.column("id", sa.Integer), sa.sql.column("passkey_id", sa.LargeBinary(length=16))
    )
    conn = op.get_bind()
    res = conn.execute(sa.sql.select([users_table]))
    for row in res:
        conn.execute(
            users_table.update().where(users_table.c.id == row["id"]).values({"passkey_id": secrets.token_bytes(16)})
        )
