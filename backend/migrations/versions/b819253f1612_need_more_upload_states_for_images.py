...
def upgrade():
    ...
    with op.batch_alter_table("image") as batch_op:
        batch_op.add_column(
            sa.Column(
                "uploadState",
                sa.Enum(
                    "initialized", "uploading", "uploaded", "failed", "broken", "completed", name="upload_state_types"
                ),
                nullable=True,
            )
        )
    ...
    if conn.engine.name == "postgresql":
        ...
        conn.execute(
            sa.text(
                "update image set \"uploadState\"=case when uploaded then 'completed'::upload_state_types else 'initialized'::upload_state_types end"
            )
        )
    else:
        conn.execute(
            sa.text("update image set \"uploadState\"=case when uploaded then 'completed' else 'initialized' end")
        )
    with op.batch_alter_table("image") as batch_op:
